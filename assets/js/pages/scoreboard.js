...
const CLASSES = {
    scoreboard: {
        ...
        teams: {
            card: "p-2 xl:p-4 my-4 text-lg xl:text-xl rounded-lg border-2 flex justify-between items-center",
    ...
    return (
        <LayoutUserDashboard location={location} history={history}>
            <div className={scoreboard.container}>
                <h1 className={scoreboard.title}>Score<span className="text-blue-500">Board</span></h1>
                ...
                {sortedTeams.map(team => {
                     return (
                        <div key={team.id + team.name} className={`${scoreboard.teams.card} ${team.id === user.team_id && scoreboard.teams.activeCard}`}>
                            <h2>{capitalize(team.name)}</h2>      
                            <div className="flex items-center">
                                <a href={team.github_repo}><img src={GithubIcon} alt="GitHub icon" className="mx-2 h-6 w-6"/></a>  
